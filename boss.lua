	...
	variables.Attacks = {
		...
		function()
			...
			for i = 1, hoop_count do
				game.Spawn(Bullet.new, cx, cy, 30, 600, nil, nil, {0.6, 0.4, 0.7}):SetDirection(angle + ((i - 1) * hoop_div))
			end
...
local function _3bulbev(self, dt)
	...
	if self.Y >= boundaries.h + (self.Size * 0.5) then
		...
		for _ = 1, 2 do
			...
			radius = radius * 0.96
			speed = speed * 1.75
			circles = math.floor(circles * 1.75)
		end
...
local function _4bulbev(self, dt)
	if self.LeaveTimer <= 0 then
		self.XVel = self.XVel + ((3 * self.Increase) * dt)
	else 
		self.LeaveTimer = self.LeaveTimer - dt
		self.XVel = self.XVel + (-self.XVel) * (10 * dt)
	end
end
...
local function _4(self, variables, dt)
	...
	elseif variables.State == 1 then
		variables.Y = variables.Y + (360 * dt)
	
		if variables.FireTimer <= 0 then
			local cx, cy = self:GetCenter()
			local bullet = game.Spawn(Bullet.new, cx, cy + variables.RandOffset, 12, 20, nil, nil)
			...
			variables.FireTimer = 0.075 * 0.5
			variables._f = not variables._f
		else variables.FireTimer = variables.FireTimer - dt
		end

		if variables.Y >= boundaries.h + (self.Height * 0.5) then
			variables.State = 2
...
local function _5(self, variables, dt)
	...
	if variables.SideTimer <= 0 then
		...
		if variables.Sides > 8 then
			variables.Sides = 3
		end

		variables.SideTimer = 3
...
local function _6(self, variables, dt)
	...
	if variables.NewGroupTimer <= 0 then
		local bullet_amount = 15
		local rotation = -1
		local radius = 150
		local radius_velocity = 100
		local start_angle = 0

		for _ = 1, 2 + (variables.Half and 1 or 0) do
			local new_group = {
				Center = {
					X = pcx;
					Y = pcy;
			...
			radius = radius * 1.25
			radius_velocity = radius_velocity * 3
	...
	for _, group in ipairs(variables.Groups) do
		for _, bullet in ipairs(group.Bullets) do
			...
			if actual.Color[4] >= 1 and not actual.CanCollide then
				...
				actual.Color[4] = actual.Color[4] + (dt * 0.5)
			end
		end
		...
		group.Radius = group.Radius + (group.RadiusVelocity * dt)
...
local function _7setup(variables, self)
	game.GameSpeed = 0.125
	-- game.ShakeIntensity = 5
	playsound("bomb")

	variables.DeathTimer = 0.75
...
local function _7(self, variables, dt)
	...
	if variables.LoseStarTimer <= 0 then
		...
		variables.LoseStarTimer = 0.0125
	else
