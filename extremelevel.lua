local boss_f = require("extremeboss")
...
local function _1bev(enemy, dt)
	...
	if enemy.ShootTimer <= 0 then
		...
		for i = 1, 40 do
			game.Spawn(Bullet.new, cx, cy, 20, 600, nil, nil, _1colors[2]):SetDirection(
...
_2states[2] = function(enemy, dt)
	...
	if enemy.FireTimer <= 0 then
		...
		elseif enemy.ShootType >= 3 then
			...
			for i = 1, 5 do
				for p = 1, 3 do
					game.Spawn(Bullet.new, cx, cy, 12, 400 + ((i - 1) * 60), nil, nil, {0.3, 0.5, 0.7, 1}):SetDirection(enemy.CurrentAngle + ((i - 1) * _2_ANGLE_INC))
				end
			end
...
local function _3sonbev(enemy, dt)
	...
	if enemy.FireTimer <= 0 then
		...
		for i = 0, 20 do
			game.Spawn(Bullet.new, cx, cy, 25, 10 + (i * 80), nil, nil, _3_BULLET_COLOR):SetDirection(dir)
		end
...
local function _4bev(self, dt)
	...
	if self.FireTimer <= 0 then
		...
		local speed = math.sin(self._s) * 400

		for i = 1, 13 do
			local bullet = game.Spawn(Bullet.new, cx, cy, 20, speed, math.random(-1.5, 1.5), -1, _4_BULLET_COLOR)
			bullet.GoSpeedTimer = 2
...
local function _5bev(self, dt)
	...
	if self.FireTimer <= 0 then
		...
		local cx, cy = self:GetCenter()
		local sine_speed = math.max(20 + math.sin(0.5 + self._mod) * 100, 100)

		for i = 1, 3 do
			local bullet = game.Spawn(Bullet.new, cx, cy, 20, sine_speed * 3, nil, nil, _5_BULLET_COLORS[self.AngleWhere])
			bullet:SetDirection(_5_START_ANGLE + (_5_MOD_EACH * mod) + math.rad(((i - 1) * 105)))
			bullet.AngleTurn = math.rad(sine_speed / 20) + math.rad(i * 25)
